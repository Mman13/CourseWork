...
FogShader::FogShader()
{
	...
	m_constantBuffer = 0;
	m_sampleState = 0;
	m_fogBuffer = 0;//Initialize the fog buffer
}
...
bool FogShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
			    D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView** textureArray, float fogStart, float fogEnd)
{
	bool result;
					 ...
					 float fogEnd)
{
	...
	//We set the fog start and fog end position in the constant buffer here by locking the buffer,
	//setting the data, and then unlocking the buffer again. It is the second buffer in the vertex shader
	//so we change the bufferNumber from 0 to 1.
