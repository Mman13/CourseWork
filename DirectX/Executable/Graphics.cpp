...
//////////////////////////////////////////////
	...
	result = m_Plane->Initialize(m_D3D->GetDevice(), L"WaterTex.dds");
	if(!result)
	{
	...
	result = m_SkyBox->Initialize(m_D3D->GetDevice(), "Sphere.obj", L"skybox.JPG", L"skybox.JPG");
	if(!result)
	{
	...
	result = m_Model->Initialize(m_D3D->GetDevice(), "Sphere.obj", L"stone01.DDS", L"stone01.DDS");
	if(!result)
	{
	...
	result = m_FloorModel->Initialize(m_D3D->GetDevice(), "ground.txt", L"marble01.dds");
	if(!result)
	{
	...
	result = m_Terrain->Initialize(m_D3D->GetDevice(), "CourseworkHeightmap.bmp", L"dirt01.dds");
	if(!result)
	{
	...
	result = m_Fire->Initialize(m_D3D->GetDevice(), "square.txt", L"fire01.dds", 
